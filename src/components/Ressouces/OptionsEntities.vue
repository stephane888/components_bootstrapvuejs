<template>
  <div>
    <b-form-group :label="field.label">
      <b-form-select v-if="field.type == 'options_select'" v-model="selected" :options="options" :name="field.name" @change="input"></b-form-select>
      <b-form-radio-group v-else v-model="selected" :options="options" :name="field.name" @change="input"></b-form-radio-group>
    </b-form-group>
  </div>
</template>
  ...
  methods: {
    loadTerms() {
      ...
      if (entity_type_id && loadField.config) {
        const terms = new itemsEntity(entity_type_id, entity_type_id, loadField.config);
        terms.get().then(() => {
    ...
    getValue() {
      if (this.model[this.field.name] && this.model[this.field.name][0]) {
        if (this.model[this.field.name][0].value) return this.model[this.field.name][0].value;
